...
export default function AdminDashboard() {
  ...
  const [queue, setQueue] = useState(pendingValidations);
  const [flash, setFlash] = useState<string | null>(null);

  if (!allowed) return null;

  function decide(title: string, approved: boolean) {
    setQueue((q) => q.filter((c) => c.title !== title));
    setFlash(`« ${title} » ${approved ? "approuvée et publiée" : "refusée"}.`);
  }
                  ...
                  <tr key={c.title} className="border-t border-line align-middle">
                    ...
                    <td className="py-4">
                      <div className="flex gap-2">
                        <button
                          onClick={() => decide(c.title, true)}
                          className="inline-flex items-center gap-1.5 rounded-full bg-success px-3.5 py-1.5 text-xs font-semibold text-white transition hover:opacity-90"
                        >
                          <CheckIcon width={14} height={14} />
                          Approuver
                        </button>
                        <button
                          onClick={() => decide(c.title, false)}
                          className="inline-flex items-center gap-1.5 rounded-full border border-danger/40 px-3.5 py-1.5 text-xs font-semibold text-danger transition hover:bg-danger-soft"
                        >

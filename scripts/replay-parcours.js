...
const STUDENT_ID = db.prepare("select id from User where email=?").get(STUDENT).id;
...
// Remise a zero des traces du rejeu precedent : sans elle le formulaire d'avis
// s'ouvre en mode "deja note" et le cours payant est deja acquis.
function reset() {
  const uid = db.prepare("select id from User where email=?").get(STUDENT).id;
  const paid = db.prepare("select id from Course where slug=?").get(PAID).id;
  db.prepare("delete from Review where userId=?").run(uid);
  db.prepare("delete from Purchase where userId=? and courseId=?").run(uid, paid);
  db.prepare("delete from LessonProgress where enrollmentId in (select id from Enrollment where userId=? and courseId=?)").run(uid, paid);
  db.prepare("delete from Enrollment where userId=? and courseId=?").run(uid, paid);
  const old = db.prepare("select id from Course where title like 'Formation rejeu%'").all();
  for (const c of old) {
    db.prepare("delete from Lesson where partId in (select id from CoursePart where courseId=?)").run(c.id);
    db.prepare("delete from CoursePart where courseId=?").run(c.id);
    db.prepare("delete from Course where id=?").run(c.id);
  }
  db.prepare("delete from CourseDraft where name like 'Formation rejeu%'").run();
  console.log(`reset: ${old.length} formation(s) de rejeu precedentes retirees`);
}
reset();
const rows = [];
...
(async () => {
  ...
  let s;
  await step("etudiant: login -> tableau de bord", async () => { s = await login(sctx, STUDENT); return s.url.includes("/tableau-de-bord") ? `HTTP ${s.status} -> ${s.url}` : ko(s.url); });
  const page = s.page;
  const freeId = q1("select id from Course where slug=?", FREE).id;

  await step("etudiant: page cours gratuit 200", async () => { const r = await page.goto(`${BASE}/fr/formations/${FREE}`); return r.status() === 200 ? "HTTP 200" : ko(`HTTP ${r.status()}`); });
  ...
  await step("etudiant: LessonProgress 25 (DB)", () => { const n = q1("select count(*) n from LessonProgress lp join Enrollment e on e.id=lp.enrollmentId where e.userId=? and e.courseId=? and lp.isCompleted=1", STUDENT_ID, freeId).n; return n === 25 ? "25" : ko(`${n}`); });
  await step("etudiant: QuizAttempt >= 6 (DB)", () => { const n = q1("select count(*) n from QuizAttempt where userId=?", STUDENT_ID).n; return n >= 6 ? `${n}` : ko(`${n}`); });
  await step("etudiant: Enrollment progress 100 + completedAt (DB)", () => { const e = q1("select progress, completedAt from Enrollment where userId=? and courseId=?", STUDENT_ID, freeId); return e && e.progress >= 100 && e.completedAt ? `progress ${e.progress}, completedAt ${e.completedAt}` : ko(JSON.stringify(e)); });
  ...
  await step("etudiant: page certificat 200 + nom", async () => { const r = await page.goto(`${BASE}/fr/formations/${FREE}/certificat`); const t = await page.locator("main").innerText(); return r.status() === 200 && t.includes("Laura Durand") ? "HTTP 200, nom present" : ko(`HTTP ${r.status()}`); });
  ...
  await step("etudiant: lecon 3 verrouillee avant achat", async () => { const r = await page.goto(`${BASE}/fr/formations/${PAID}/${third.key}`); const t = await page.locator("main").innerText(); return r.status() === 200 && t.includes(LOCKED) ? `HTTP 200, verrou visible (${third.key})` : ko(`HTTP ${r.status()} verrou ${t.includes(LOCKED)}`); });
  await step("etudiant: bouton Acheter -> action serveur (checkoutFailed)", async () => {
    ...
    // La region live [role="alert"] existe des le chargement, vide : attendre sa
    // presence ne prouve rien, il faut attendre qu'elle porte un texte.
    ...
    return t.includes("Le paiement n'a pas pu démarrer") ? "alerte checkoutFailed" : ko(t);
  });
  await step("webhook: checkout.session.completed signe -> 200", async () => {
    ...
    return r.status === 200 ? `HTTP 200 ${txt.slice(0, 60)}` : ko(`HTTP ${r.status} ${txt.slice(0, 120)}`);
  });
  await step("achat: Purchase paid + Enrollment (DB)", () => { const p = q1("select status, amountCents from Purchase where userId=? and courseId=?", STUDENT_ID, paid.id); const e = q1("select 1 from Enrollment where userId=? and courseId=?", STUDENT_ID, paid.id); return p && p.status === "paid" && e ? `paid ${p.amountCents} + enrollment` : ko(JSON.stringify(p)); });
  await step("achat: banniere ?achat=ok + Formation acquise", async () => { await page.goto(`${BASE}/fr/formations/${PAID}?achat=ok`); const t = await page.locator("main").innerText(); return t.includes("Paiement confirmé") && t.includes("Formation acquise") ? "les 2 textes presents" : ko(`confirme ${t.includes("Paiement confirmé")} acquise ${t.includes("Formation acquise")}`); });
  await step("achat: lecon 3 deverrouillee", async () => { const r = await page.goto(`${BASE}/fr/formations/${PAID}/${third.key}`); const t = await page.locator("main").innerText(); return r.status() === 200 && !t.includes(LOCKED) ? "HTTP 200, contenu accessible" : ko(`HTTP ${r.status()} verrou ${t.includes(LOCKED)}`); });
  ...
  await step("etudiant: profil nom modifie puis retabli", async () => {
    ...
    return n1 === "Laura Durand Rejeu" && n2 === "Laura Durand" ? "DB: modifie puis retabli" : ko(`${n1} / ${n2}`);
  });
  ...
  await step("etudiant: relogin mot de passe seed", async () => { const c2 = await browser.newContext(); const r = await login(c2, STUDENT); await c2.close(); return r.url.includes("/tableau-de-bord") ? "OK" : ko(r.url); });

  const ictx = await browser.newContext();
  let ins;
  await step("formateur: login -> /formateur", async () => { ins = await login(ictx, INSTR); return ins.url.includes("/formateur") ? `HTTP ${ins.status} -> ${ins.url}` : ko(ins.url); });
  const ip = ins.page;
  await step("formateur: PayoutsPanel affiche la vente", async () => { await ip.goto(`${BASE}/fr/formateur`); const t = await ip.locator("main").innerText(); return t.includes("Votre part") && t.toLowerCase().includes("cybers") ? "vente listee" : ko(`part ${t.includes("Votre part")} vente ${t.toLowerCase().includes("cybers")}`); });
  ...
  await step("formateur: Envoyer la demande -> /formateur + SUBMITTED", async () => {
    ...
    return d.status === "SUBMITTED" && cur.length === 1 && cur[0].lessons.length === 2 ? "SUBMITTED, 1 module, 2 lecons" : ko(`${d.status} ${(d.curriculum || "").slice(0, 80)}`);
  });
  ...
  let ad;
  await step("admin: login -> /admin", async () => { ad = await login(actx, ADMIN); return ad.url.includes("/admin") ? `HTTP ${ad.status} -> ${ad.url}` : ko(ad.url); });
  const ap = ad.page;
